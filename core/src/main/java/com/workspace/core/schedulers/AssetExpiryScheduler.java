...
public class AssetExpiryScheduler implements Runnable {
        ...
        private static final String SUBSERVICE = "asset-expiry-service";

        private static final int DEFAULT_WINDOW_DAYS = 7;

        private static final String DEFAULT_DAM_ROOT_PATH = "/content/dam/workspace";
        ...
        @ObjectClassDefinition(name = "Workspace Asset Expiry Scheduler Configuration", description = "Configuration for the DAM asset expiry report scheduler")
        public @interface Config {

                @AttributeDefinition(name = "Enabled", description = "Enable or disable asset expiry notifications")
                boolean enabled() default true;

                @AttributeDefinition(name = "Cron Expression", description = "Quartz cron expression used to run the scheduler")
                String cronExpression() default "0 0 9 * * ?";

                @AttributeDefinition(name = "Notification Window Days", description = "Number of calendar days ahead to check for expiring assets")
                int notificationWindowDays() default 7;

                @AttributeDefinition(name = "DAM Root Path", description = "DAM root path under which expiring assets are searched")
                String damRootPath() default "/content/dam/workspace";

                @AttributeDefinition(name = "AEM Author URL", description = "Base URL used to construct Author asset links")
                String authorUrl() default "http://localhost:4502";

                @AttributeDefinition(name = "Deduplication Enabled", description = "Prevent duplicate report entries for the same asset and expiration date")
        ...
        protected void activate(Config config) {
                ...
                notificationWindowDays = config.notificationWindowDays();
                ...
                if (notificationWindowDays < 0) {
                        ...
                        notificationWindowDays = DEFAULT_WINDOW_DAYS;
                }
                ...
                if (damRootPath == null
                        ...
                        damRootPath = DEFAULT_DAM_ROOT_PATH;
                }

                damRootPath = damRootPath.trim();
        ...
        protected void execute() {
                ...
                Instant windowEnd = windowStart.plus(
                                notificationWindowDays,
                                ChronoUnit.DAYS);
                ...
                try {
                        ...
                        expiringAssetPaths = assetExpiryQueryService.findExpiringAssets(
                                        damRootPath,
                                        windowStart,
                                        windowEnd);
                ...
                foundCount = expiringAssetPaths.size();
                ...
                try {
                        ...
                        recipientList = damAdminRecipientService.getRecipientEmails();
                 ...
                 */
                List<Resource> reportAssets = new ArrayList<Resource>();

                List<LocalDate> reportExpirationDates = new ArrayList<LocalDate>();

                List<AssetExpiryReportEntry> reportEntries = new ArrayList<AssetExpiryReportEntry>();

                try (ResourceResolver resolver = getServiceResourceResolver()) {

                        LocalDate today = LocalDate.now();

                        LocalDate lastEligibleDate = today.plusDays(
                                        notificationWindowDays);

                        for (String assetPath : expiringAssetPaths) {
                                ...
                                try {
                                        ...
                                        Resource asset = resolver.getResource(
                                                        assetPath);
                        ...
                        try {
                                 ...
                                 * Generate CSV.
                                 */
                                reportFile = assetExpiryReportService.generateReport(
                                                reportEntries,
                                                "author",
                                                authorUrl);
                                 ...
                                 */
                                boolean reportSent = assetExpiryNotificationService
                                                .sendExpiryReport(
                                                                reportFile,
                                                                recipients,
                                                                reportCount);
                                ...
                                if (deduplicationEnabled) {
                                        ...
                                        for (int i = 0; i < reportAssets.size(); i++) {

                                                try {

                                                        boolean auditSaved = expiryNotificationAuditService
                        ...
                        failedCount += expiringAssetPaths.size();
                }
        ...
        private LocalDate getExpirationDate(
                ...
                Resource metadata = asset.getChild(
                ...
                java.util.Calendar expirationCalendar = metadata.getValueMap().get(
